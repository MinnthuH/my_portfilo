                            ...
                            <form action="{{ route('update#blog') }}" method="POST" enctype="multipart/form-data">
                                        ...
                                        <select class="form-select" aria-label="Default select example"
                                            ...
                                            @foreach ($category as $item)
                                                <option value="{{ $item->id }}"
                                                    {{ $item->id == $blogs->blog_category_id ?  'selected': ''}}>{{ $item->blog_category }}
                                                </option>
                                            @endforeach
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-10">
                                        <input class="form-control" type="text" name="blogTag" id="example-text-input"
                                            value="{{ $blogs->blog_tag }}" data-role="tagsinput">
                                ...
                                <div class="row mb-3">
                                    <label for="example-text-input" class="col-sm-2 col-form-label">Blog
                                        Description</label>
                                    <div class="col-sm-10">
                                        <textarea id="elm1" name="blogDescription">{{ $blogs->blog_description }}</textarea>
                                    </div>
                                </div>
                                <!-- end row -->
                                <div class="row mb-3">
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-sm-10">
                                        <img class="rounded avatar-lg" id="showImage" src="{{ asset($blogs->blog_image) }}"
                                            alt="Card image cap">
                                    </div>
                                </div>
                                <!-- end row -->
